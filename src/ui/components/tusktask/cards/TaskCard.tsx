...
const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <div className="group cursor-pointer active:scale-95 transition-all duration-300 px-4 py-2 border rounded-2xl overflow-hidden shadow hover:shadow-xl space-y-2">
      <header className="grid grid-cols-[30px_auto] gap-2">
        ...
        <div className="flex justify-between items-center">
          <h4 className="tracking-tight text-sm font-semibold">{name}</h4>
      ...
      <p className="text-xs text-tt-primary-foreground/70">{description}</p>
      <Separator />
